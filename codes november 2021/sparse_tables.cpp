...
int main(){
	...
	// lot of logs ->we crearte a log table
	...
	while(1){
		...
		// min of interval indices [l,r) dont inclide r to ans l -r
		// min of v[l] v[l+1]..v[r-1]
		
		int log=log_table[r-l];
		
		int ans=min(sparse_table[log][l],sparse_table[log][r-(1<<log)]);
		cout<<ans;
